...
class Agent():
    ...
    def train(self,env, state, action):

        # Select final goal from final goal space, defined in "design_agent_and_env.py"
        self.goal_array[self.FLAGS.layers - 1] = env.get_next_goal(self.FLAGS.test, state, action)
        print("\nNext Action: ", action)
        print("Skill Goal State: ", self.goal_array[self.FLAGS.layers - 1])
        ...
        goal_status, max_lay_achieved = self.layers[self.FLAGS.layers-1].train(self,env)
